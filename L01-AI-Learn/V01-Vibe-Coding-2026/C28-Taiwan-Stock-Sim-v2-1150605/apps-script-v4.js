...
const QUOTE_WATCHLIST_SHEET_NAME = 'quote_watchlist';
const HEADERS = [
...
const TW_STOCK_NAME_MAP = {
  '2330':'台積電','2317':'鴻海','2454':'聯發科','2882':'國泰金','2412':'中華電','0050':'元大台灣50','2308':'台達電','1301':'台塑','2002':'中鋼','2303':'聯電','2881':'富邦金','2886':'兆豐金','2891':'中信金','2892':'第一金','2884':'玉山金','2885':'元大金','2887':'台新金','2888':'新光金','2890':'永豐金','2357':'華碩','2382':'廣達','2395':'研華','6505':'台塑化','1303':'南亞','1326':'台化','2207':'和泰車','2408':'南亞科','3008':'大立光','2379':'瑞昱','6669':'緯穎','3711':'日月光投控','2823':'中壽','2801':'彰銀','2834':'台企銀','2838':'聯邦銀','5880':'合庫金','2849':'安泰銀','2867':'三商壽','5871':'中租-KY','2327':'國巨','3034':'聯詠','3037':'欣興','2301':'光寶科','2347':'聯強','2360':'致茂','2385':'群光','2451':'創見','2458':'義隆','2492':'華新科','2498':'宏達電','3045':'台灣大','4904':'遠傳','4938':'和碩','6415':'矽力-KY','6446':'藥華藥','8046':'南電','9910':'豐泰','1229':'聯華','1216':'統一','2912':'統一超','2915':'潤泰全','5904':'寶雅','0051':'元大中型100','0052':'富邦科技','0056':'元大高股息','00878':'國泰永續高股息','00881':'國泰台灣5G+','00919':'群益台灣精選高息'
};
...
function getQuoteWatchlistSheet() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheet = ss.getSheetByName(QUOTE_WATCHLIST_SHEET_NAME);
  if (!sheet) {
    sheet = ss.insertSheet(QUOTE_WATCHLIST_SHEET_NAME);
    sheet.appendRow(['股票代號','股票名稱','建立時間','更新時間']);
    sheet.setFrozenRows(1);
...
function doGet(e) {
  try {
    ...
    if (action === 'getQuoteWatchlist') return jsonResp(getQuoteWatchlist());
...
function doPost(e) {
  try {
    ...
    if (p.action === 'saveQuoteWatch') return jsonResp(saveQuoteWatch(p));
    return jsonResp({ status: 'error', message: '未知 action' });
  } catch (err) {
    return jsonResp({ status: 'error', message: err.toString() });
  }
...
function getQuoteWatchlist() {
  const sheet = getQuoteWatchlistSheet();
  const data = sheet.getDataRange().getValues();
  data.shift();
  const rows = data
    .map(r => ({
      code: normalizeStockCode(r[0]),
      name: getTraditionalStockName(r[0], r[1]),
      createdAt: r[2] || '',
      updatedAt: r[3] || ''
    }))
    .filter(r => r.code);
...
function saveQuoteWatch(p) {
  const code = normalizeStockCode(p.code);
  if (!code) return { status: 'error', message: '缺少股票代號' };

  const quote = getStockQuote(code);
  const name = getTraditionalStockName(code, p.name || (quote && quote.name));
  const sheet = getQuoteWatchlistSheet();
  const data = sheet.getDataRange().getValues();
  const ts = nowTW();

  for (let i = 1; i < data.length; i++) {
    if (normalizeStockCode(data[i][0]) === code) {
      sheet.getRange(i + 1, 2).setValue(name);
      sheet.getRange(i + 1, 4).setValue(ts);
      return { status: 'ok', code, name, quote: quote.status === 'ok' ? quote : null, message: '即時報價清單已更新' };
    }
  }

  sheet.appendRow([code, name, ts, ts]);
  return { status: 'ok', code, name, quote: quote.status === 'ok' ? quote : null, message: '已加入即時報價清單' };
}
...
function addBuy(p) {
  const sheet     = getSheet();
  const ts        = nowTW();
  const stockName = getTraditionalStockName(p.code, p.name);
...
function addSell(p) {
  ...
  const stockName = getTraditionalStockName(buyData[2], buyData[3]);
...
function getStockQuote(code) {
  ...
  if (cached) return localizeQuote(JSON.parse(cached));
  ...
  const payload = localizeQuote(Object.assign({ status: 'ok' }, quote, {
...
function getStockQuotes(codes) {
  ...
  normalized.forEach(code => {
    const quote = localizeQuote(twseQuotes[code] || yahooTwQuotes[code] || yahooTwoQuotes[code] || googleQuotes[code] || getStockQuote(code));
...
function getTraditionalStockName(code, fallback) {
  const normalized = normalizeStockCode(code);
  const name = TW_STOCK_NAME_MAP[normalized] || String(fallback || normalized);
  return name.replace(/\s+(Inc\.?|Corporation|Corp\.?|Company|Co\.?|Ltd\.?|Limited|ADR|Common Stock).*$/i, '').trim() || normalized;
}

function localizeQuote(quote) {
  if (!quote) return quote;
  const code = normalizeStockCode(quote.code);
  if (!code) return quote;
  quote.code = code;
  quote.name = getTraditionalStockName(code, quote.name);
  return quote;
}
